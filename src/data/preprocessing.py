...
def rotate_image(img: np.ndarray, angle: float, pivot: Tuple[int, int]) -> np.ndarray:
    ...
    img_padded = np.pad(
        img, [pad_y, pad_x, [0, 0]], "constant", constant_values=img[0, 0, 0]
    )
...
def crop_image(
    img: np.ndarray, width: int, height: int, point: Tuple[int, int]
) -> np.ndarray:
    """
    ...
    cropped = img[
        y_center - height : y_center, x_center - width // 2 : x_center + width // 2
    ]

    return cropped


def point_angle_to_map(
    point: Tuple[int, int], angle: float, img: np.ndarray, size: int = 150
) -> np.ndarray:
    """
